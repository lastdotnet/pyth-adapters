...
contract PythAdapterFactory is AccessControl {
     ...
     */
    constructor(address _pythPriceFeedsContract, address[] memory assets, bytes32[] memory priceIds, string[] memory descriptions) {
     ...
     */
    function deployPythAdapter(bytes32 priceId, address asset,string memory description) public onlyAdminOrDeployer returns (address adapter) {
        require(feedIdToAdapter[priceId] == address(0), "Adapter already exists");
        
        // Deploy new adapter
        PythAggregatorV3 newAdapter = new PythAggregatorV3(
     ...
     */
    function batchDeployPythAdapters(
        bytes32[] memory priceIds,
        address[] memory assets,
        string[] memory descriptions
    ) public onlyAdminOrDeployer returns (address[] memory adapters) {
